...
**  @file               display_ctrl.h
**
**  @brief              Digilent Display Controller Driver.
**  @par		This module provides an easy to use API for controlling the
...
**  @copyright			Copyright 2014, Digilent Inc.
**
*******************************************************************************
**
**  @par VERSION HISTORY
...
**  -------- | ----------- | ----------------- | ----------------------------
**  0.1.0    | 2014-02-20  | Sam Bobrowicz     | File creation
**  1.0.0    | 2017-11-15  | Javier Peces      |
**
*******************************************************************************
**/
...
/**
*******************************************************************************************
** @brief   DisplayStop - Halts output to the display
**
** @param       dispPtr(DisplayCtrl)	Pointer to the initialized DisplayCtrl struct
** @return      int		XST_SUCCESS if successful.
...
/**
*******************************************************************************************
** @brief   DisplayStart - Starts the display.
**
** @param       dispPtr(DisplayCtrl)	Pointer to the initialized DisplayCtrl struct
...
/**
*******************************************************************************************
** @brief   DisplayInitialize - Initializes the driver struct for use.
**
** @param       dispPtr(DisplayCtrl)	Pointer to the initialized DisplayCtrl struct
** @param       vdmaId(u16)				DEVICE ID of the attached VDMA core
** @param       dispCtrlAddr(u32)		BASE ADDRESS of the axi_dispctrl core
** @param       fHdmi(int)				flag indicating if the C_USE_BUFR_DIV5 parameter
** 										is set for the axi_dispctrl core.
** 										Use DISPLAY_HDMI if it is set,
** 										otherwise use DISPLAY_NOT_HDMI
** @param       framePtr(u32)			array of pointers to the framebuffers.
** 										The framebuffers must be instantiated above
** 										this driver
** @param       stride(u32)				line stride of the framebuffers. This is the number
** 										of bytes between the start of one line and the start
** 										of another.
**
** @return      int		XST_SUCCESS if successful, XST_FAILURE otherwise
**
...
/**
*******************************************************************************************
** @brief   DisplaySetMode
** @par		Changes the resolution being output to the display. If the display
**			is currently started, it is automatically stopped (DisplayStart must
**			be called again).
**
** @param       dispPtr(DisplayCtrl)	Pointer to the initialized DisplayCtrl struct
** @param       newMode(VideoMode)		The VideoMode struct describing the new mode
** @return      int		XST_SUCCESS if successful, XST_FAILURE otherwise
**
*******************************************************************************************
**/
...
/**
*******************************************************************************************
** @brief   DisplayChangeFrame - Changes the frame currently being displayed.
**
** @param       dispPtr(DisplayCtrl)	Pointer to the initialized DisplayCtrl struct
** @param       frameIndex(u32)			Index of the framebuffer to change to (must
